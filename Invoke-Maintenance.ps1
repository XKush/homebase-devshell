...
& "$PSScriptRoot\Validate-Workstation.ps1" -StartupBudgetMs 650 | Out-Null
$healthOk = ($LASTEXITCODE -eq 0)

# 2. Log cleanup
Write-WorkstationStep 'Log rotation'
& "$PSScriptRoot\Invoke-Housekeeping.ps1" -WhatIf:$WhatIf
...
if ($Full -or $backupAge -gt 7) {
    ...
    & "$PSScriptRoot\Backup-Configuration.ps1" -Force
}
