...
& (Join-Path $PSScriptRoot 'Sync-WorkstationDocs.ps1') -CheckOnly
if ($LASTEXITCODE -ne 0) {
    & (Join-Path $PSScriptRoot 'Sync-WorkstationDocs.ps1') | Out-Null
}
...
if ($Backup) {
    ...
    if ($days -gt 7) {
        ...
        & (Join-Path $PSScriptRoot 'Backup-Configuration.ps1') -Force
    } else {
