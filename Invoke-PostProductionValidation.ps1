...
if ($wt.profiles.defaults.font.face -ne 'CaskaydiaCove NF') {
    ...
    & "$PSScriptRoot\Repair-WorkstationAll.ps1"
}

# Validation
& "$PSScriptRoot\Validate-Workstation.ps1" -StartupBudgetMs $StartupBudgetMs
$valOk = ($LASTEXITCODE -eq 0)

# Acceptance
& "$PSScriptRoot\Invoke-AcceptanceTest.ps1" -StartupBudgetMs $StartupBudgetMs
